  ...
  >
    <div class="wb-sidebar-header">
      ...
      <div class="wb-sidebar-tools">
        <button class="wb-btn" :disabled="!canBind" @click="store.regexBindSelected()">
    ...
    <div class="wb-list" ref="listRef">
      ...
      <template v-for="(node, gi) in store.regexFlatNodes" :key="nodeKey(node, gi)">
        <div
          ...
          class="wb-tree-group"
          :class="{
            selected: store.regexSelectedGi.has(gi),
        ...
        >
          ...
          <span
            ...
            :title="uiStore.t('regex.sidebar.toggleTitle')"
            @click.stop="store.regexToggleBlock(gi)"
          ></span>
          <span
...
const canBind = computed(() => {
  ...
  const topLevel = Array.from(s.regexSelectedGi).filter(
    (gi) => s.regexFlatNodes[gi]?.parent === s.regexOrder
  );
