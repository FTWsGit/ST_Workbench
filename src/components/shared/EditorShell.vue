<template>
  <div class="wb-editor-panel" v-if="!tabsStore.activeTab">
    <div class="wb-editor-empty">
      ...
      <p v-else-if="tabsStore.sidebarCollection === 'tavern'">
        {{ uiStore.t('tavern.editorShell.empty') }}
      </p>
      <p v-else-if="tabsStore.activeWorkspace === 'worldbook'">
        {{
          worldbookStore.hasData
            ? uiStore.t('worldbook.editorShell.emptyEntry')
            : uiStore.t('worldbook.editorShell.empty')
        }}
      </p>
      <p v-else-if="tabsStore.activeWorkspace === 'character'">
        {{
          characterStore.hasData
            ? uiStore.t('character.editorShell.emptyField')
            : uiStore.t('character.editorShell.empty')
        }}
      </p>
      <p v-else-if="presetStore.hasData">
        {{ uiStore.t('preset.editorShell.empty') }}
      </p>
...
const EDITOR_COMPONENTS: Record<string, Component> = {
