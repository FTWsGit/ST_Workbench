<template>
  ...
  >
    <div class="wb-preset-vp-header">
      <span class="wb-preset-vp-varname">{{ uiStore.varPopupVarName }}</span>
      <span class="wb-preset-vp-scope">{{
        uiStore.t(
          uiStore.varPopupScope === 'local' ? 'preset.varPopup.local' : 'preset.varPopup.global'
        )
      }}</span>
      <span class="wb-preset-vp-count">{{
        uiStore.t(
      ...
      <button class="wb-preset-vp-btn" aria-label="上一个匹配" @click="uiStore.navPopupVar(-1)">
        ◀
      </button>
      <button class="wb-preset-vp-btn" aria-label="下一个匹配" @click="uiStore.navPopupVar(1)">
        ▶
      </button>
      <button
