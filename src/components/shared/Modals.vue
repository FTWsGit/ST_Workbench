<template>
  ...
  >
    <div class="wb-modal lg">
      ...
      <div class="wb-modal-scroll">
        ...
        <div class="wb-settings-section">
          ...
          <div v-for="(labelKey, key) in SYNTAX_LABEL_KEYS" :key="key" class="wb-color-row">
            <input
              type="color"
              v-model="draftColors[key as keyof SyntaxColors]"
              @change="commitColor(key as keyof SyntaxColors)"
            />
      ...
      <div class="wb-modal-footer">
        <button class="wb-btn" @click="uiStore.resetSettings()">
          {{ uiStore.t('shared.settings.resetDefaults') }}
        </button>
        <button class="wb-btn accent" @click="uiStore.settingsOpen = false">
  ...
  <div v-if="confirmStore.open" class="wb-modal-overlay" @click.self="confirmStore.cancel()">
    <div class="wb-modal sm">
      ...
      <div class="wb-modal-footer">
        <button class="wb-btn" @click="confirmStore.cancel()">
          {{ confirmStore.cancelText }}
        </button>
        <button
          class="wb-btn accent"
          :class="{ 'wb-confirm-danger': confirmStore.danger }"
  ...
  <div
    v-if="confirmStore.promptOpen"
    class="wb-modal-overlay"
    @click.self="confirmStore.cancelPrompt()"
  >
    <div class="wb-modal sm">
      <h3>{{ confirmStore.promptTitle }}</h3>
      <p v-if="confirmStore.promptMessage" class="wb-confirm-text">
        {{ confirmStore.promptMessage }}
      </p>
      <input
        type="text"
      ...
      />
      <div class="wb-modal-footer">
        <button class="wb-btn" @click="confirmStore.cancelPrompt()">
          {{ confirmStore.promptCancelText }}
        </button>
        <button class="wb-btn accent" @click="confirmStore.confirmPrompt()">
          {{ confirmStore.promptConfirmText }}
        </button>
      </div>
    </div>
  </div>
  ...
  <div
    v-if="confirmStore.multiOpen"
    class="wb-modal-overlay"
    @click.self="confirmStore.cancelMulti()"
  >
    <div class="wb-modal sm">
      <h3>{{ confirmStore.multiTitle }}</h3>
      <!-- eslint-disable vue/no-v-html -->
      <p
        v-if="confirmStore.multiMessage"
        class="wb-confirm-text"
        v-html="confirmStore.multiMessage"
      ></p>
      ...
      <div class="wb-modal-footer">
        <button class="wb-btn" @click="confirmStore.cancelMulti()">
          {{ confirmStore.multiCancelText }}
        </button>
        <button
          class="wb-btn accent"
          :class="{ 'wb-confirm-danger': confirmStore.multiDanger }"
          @click="confirmStore.confirmMulti()"
        >
          {{ confirmStore.multiConfirmText }}
        </button>
      </div>
    </div>
  </div>
