<template>
  ...
  >
    <option v-if="orphan" :value="orphan.id" disabled>
      {{ orphan.label }}
    </option>
    <option v-for="it in sel.items" :key="it.id" :value="it.id">
      {{ it.label }}
    </option>
  </select>
...
function onSelect(e: Event) {
  ...
  if (adapter.dirty()) {
    confirmStore.ask({
      ...
      message: uiStore.t(`${adapter.key}.confirm.switch.message` as LocaleKey, {
