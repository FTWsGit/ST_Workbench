<template>
  ...
  >
    ...
    <div class="wb-toolbox-body">
      ...
      <p v-if="!currentTool" class="wb-muted wb-toolbox-empty">
        {{ uiStore.t('toolbox.empty') }}
      </p>
  ...
  <div
    v-else
    class="wb-toolbox-panel"
    :class="{ float: mode === 'overlay' }"
    :style="{ width: uiStore.settings.toolBoxWidth + 'px' }"
  >
    <div
...
const toolProps = computed<Record<string, unknown>>(() => ({
  scene: scene.value,
