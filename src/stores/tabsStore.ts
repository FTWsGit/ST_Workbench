...
export const useTabsStore = defineStore('tabs', () => {
  ...
  const sidebarCollectionByWorkspace = ref<Record<string, string>>({
    preset: 'items',
    character: 'fields',
  });
  ...
  function setSidebarCollection(workspace: string, collection: string) {
    sidebarCollectionByWorkspace.value[workspace] = collection;
  }

  /** 工具箱面板开关，按 workspace 分桶。工具箱是跨 workspace 通用的（preset/worldbook/character 都能开）。 */
  ...
  function setToolBoxOpen(workspace: string, open: boolean) {
    toolBoxOpenByWorkspace.value[workspace] = open;
  }
