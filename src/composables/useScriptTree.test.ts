...
describe('useScriptTree - 薄包装委托 useScriptList', () => {
  function setup(scriptsRef: ReturnType<typeof ref<Script[] | null>>) {
  ...
  it('addScriptTree 创建 Script（enabled=true, 带默认字段）', () => {
  ...
  it('未加载时 showToast 并返回 null', () => {
    const scriptsRef = ref<Script[] | null>(null);
    const u = setup(scriptsRef);
  ...
  it('reorderScriptTree 重排', () => {
    const scriptsRef = ref<Script[]>([makeScript('a'), makeScript('b'), makeScript('c')]);
    const u = setup(scriptsRef);
