# start_relay.ps1 - start relay.py as a detached background process via WMI.
#
...
#   console window at all. relay.py redirects stdout/stderr to devnull when
#   they are None (as they are under pythonw), so it runs fine.
#
# Usage (called by cli.py, not meant to be run by hand):
#   powershell -NoProfile -ExecutionPolicy Bypass -File start_relay.ps1 `
#       -PythonPath <full path to python.exe> -RelayPath <full path to relay.py> -Port 8791
#
...
$cmdline = '"{0}" "{1}" {2}' -f $PythonW, $RelayPath, $Port
